...
import type { SmartMarkupScenario } from '@/types';
...
function ScenarioCard({ title, scenario, emptyHint }: ScenarioCardProps) {
  return (
      ...
      {!scenario ? (
        ...
        <>
          <p className="mb-2 text-2xl font-bold tracking-tight text-primary">
            {formatCurrency(scenario.idealPrice)}
          </p>
          ...
          <ResultRow label="CPA Máximo" value={formatCurrency(scenario.maxCpa)} />
...
export function SmartMarkupResults({ byProfit, byMargin }: SmartMarkupResultsProps) {
  return (
